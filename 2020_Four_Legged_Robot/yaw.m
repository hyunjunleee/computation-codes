...
for theta = 0:sign(t):t
  L0(abs(theta)+1,1,:) = actuate(1, -L / 2 + L * cos(deg2rad(theta)) / 2 + W * sin(deg2rad(theta)) / 2, -H, W / 2 + L * sin(deg2rad(theta)) / 2 - W * cos(deg2rad(theta)) / 2);
  L0(abs(theta)+1,2,:) = actuate(2, -L / 2 + L * cos(deg2rad(theta)) / 2 - W * sin(deg2rad(theta)) / 2, -H, -W / 2 + L * sin(deg2rad(theta)) / 2 + W * cos(deg2rad(theta)) / 2);
  L0(abs(theta)+1,3,:) = actuate(3, L / 2 - L * cos(deg2rad(theta)) / 2 + W * sin(deg2rad(theta)) / 2, -H, W / 2 - L * sin(deg2rad(theta)) / 2 - W * cos(deg2rad(theta)) / 2);
  L0(abs(theta)+1,4,:) = actuate(4, L / 2 - L * cos(deg2rad(theta)) / 2 - W * sin(deg2rad(theta)) / 2, -H, -W / 2 - L * sin(deg2rad(theta)) / 2 + W * cos(deg2rad(theta)) / 2);
end
L1 = embody(L0, t);
